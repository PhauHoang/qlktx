	...
	switch ($action) {
		case 'duyet':
            ...
            if($rs){
                    ...
                    $sql_update_pmoi = "update phong set SoNguoiHienTai=(SoNguoiHienTai +1) where MaPhong='$mapc'";
                    $rs_update_pmoi = mysqli_query($conn, $sql_update_pmoi);
          			$sql="update chitietdangky set MaPhong='$mapc' where MaDK=$madk";
          			$rs=mysqli_query($conn,$sql);
          			if($rs){
              			    $sql="DELETE FROM chitietchuyenphong WHERE MaDK=$madk";
      ...
      $admin=$_SESSION['nv_admin'];$manv=$admin['MaNV'];
        ...
        if($rs){
            ...
            $sql="update chitietchuyenphong set MaPhongChuyen=NULL ,TinhTrang=NULL, LanChuyen=(LanChuyen-1) where MaDK=$madk";
